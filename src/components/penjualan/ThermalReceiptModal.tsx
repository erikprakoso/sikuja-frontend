        ...
        <div className="bg-white text-black font-mono text-xs p-4 rounded-xl shadow-inner border border-gray-300 max-h-96 overflow-y-auto w-[260px] mx-auto space-y-3 leading-tight">
          ...
          <div className="space-y-2.5 my-2">
            {physicalVouchers.map((v, idx) => (
              <div key={v.code} className="border border-black p-2 text-center rounded bg-gray-50 space-y-0.5">
                <p className="text-[9px] font-bold uppercase text-gray-700">#KUPON FISIK {idx + 1}</p>
                <p className="text-2xl font-black font-mono tracking-widest my-0.5 text-black">{v.code}</p>
                <p className="text-[8px] text-gray-600 font-sans">Scan di Pos Rute Jalan Sehat!</p>
              </div>
            ))}
          </div>
