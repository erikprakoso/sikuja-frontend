...
export const TransactionResult: React.FC<TransactionResultProps> = ({
  ...
  return (
    <div className="bg-white border border-[#E70013]/20 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6 animate-fade-in">
      {/* Header Bar */}
      <div className="flex items-center gap-3.5 pb-4 border-b border-[#E70013]/15">
        <div className="w-11 h-11 rounded-2xl bg-[#E70013] text-white flex items-center justify-center flex-shrink-0 shadow-sm">
          <CheckCircle2 className="w-6 h-6" />
        </div>
        <div>
          <h2 className="text-lg font-black text-[#E70013] leading-tight">Transaksi Berhasil</h2>
          <p className="text-xs text-[#E70013]/70 font-semibold mt-0.5">
      ...
      <div className={`grid gap-5 ${transaction.qty_non_fisik > 0 && transaction.qty_fisik > 0 ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'}`}>
        ...
        {transaction.qty_non_fisik > 0 && (
          <div className="bg-white border border-[#E70013]/20 rounded-2xl p-5 text-center space-y-3.5 flex flex-col items-center justify-center">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#E70013] text-white text-xs font-bold">
            ...
            <p className="text-xs text-[#E70013]/70 font-medium leading-relaxed max-w-xs">
            ...
            <div className="flex items-center gap-2 w-full max-w-xs pt-1">
              <button
                onClick={onCopyLink}
                className="flex-1 py-2 px-3 rounded-xl bg-white border border-[#E70013]/30 text-[#E70013] text-xs font-bold flex items-center justify-center gap-1.5 transition-colors cursor-pointer active:scale-95 shadow-sm hover:bg-[#E70013] hover:text-white"
              >
              ...
              <a
                ...
                className="p-2.5 rounded-xl bg-[#E70013] text-white transition-colors cursor-pointer active:scale-95 shadow-sm"
        ...
        {transaction.qty_fisik > 0 && (
          <div className="bg-white border border-[#E70013]/20 rounded-2xl p-5 space-y-3.5">
            <div className="flex items-center justify-between">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#E70013] text-white text-xs font-bold">
                <Ticket className="w-3.5 h-3.5" />
                Daftar Kupon Fisik ({transaction.qty_fisik} Lembar)
              </div>
              <span className="text-[11px] text-[#E70013]/60 font-semibold">Salin ke kupon fisik</span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 max-h-52 overflow-y-auto pr-1">
              {physicalVouchers.map((v, idx) => (
                <div
                  key={v.code}
                  className="p-3 bg-[#E70013]/5 border border-[#E70013]/20 rounded-xl text-center shadow-2xs"
                >
                  <span className="text-[10px] text-[#E70013]/60 font-mono font-bold block">#Kupon {idx + 1}</span>
                  <span className="text-xl font-black text-[#E70013] font-mono tracking-wider block mt-0.5">
                    {v.code}
                  </span>
                </div>
              ))}
      ...
      {/* Action Footer Bar */}
      <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-[#E70013]/15">
        <button
          onClick={onOpenPrintModal}
          className="w-full sm:w-auto py-2.5 px-4 rounded-xl bg-white border border-[#E70013]/30 text-[#E70013] hover:bg-[#E70013]/10 font-bold text-xs flex items-center justify-center gap-2 transition-all cursor-pointer active:scale-95 shadow-xs"
        >
